...
export default class ClubView extends Component {
	...
	_getDatas = () => {
		...
		// 데이터 가져오기
		axios
			.post('http://dkstkdvkf00.cafe24.com/GetClubChars.php', {
				clubName: clubName,
				school: school,
			})
			.then(result => {
				...
				<TouchableOpacity onPress={this.showOverlay.bind(this)} style={styles.club}>
					...
					<Text style={styles.clubChar}>
						{clubChar.map((chars, i) => {
							return <ClubChars chars={clubChar[i]} key={i} />;
						})}
					</Text>
				</TouchableOpacity>
				...
				>
					<View style={{ flexDriection: 'column' }}>
						<View style={{ flexDirection: 'row' }}>
							<View style={styles.logo}>
								{clubLogo === null || clubLogo == '' ? (
									...
									{ clubLogo } && <Image source={{ uri: clubLogo }} style={styles.Image} />
								)}
							</View>
						...
						<View style={{ flexDirection: 'row', justifyContent: 'center', alignItems: 'center' }}>
							<TouchableOpacity style={styles.button} onPress={this._gotoClubIntroduce}>
								<Image style={styles.ImageR} source={require('../images/introduce.png')} />
								<Text style={{ textAlign: 'center', fontSize: 15 }}>소개</Text>
							</TouchableOpacity>
							<TouchableOpacity style={styles.button} onPress={this._gotoRecord}>
