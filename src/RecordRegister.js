...
export default class RecordRegister extends React.Component {
	...
	_deletePrevDatas = async () => {
		...
		const t = this;
		await axios
			.post('http://dkstkdvkf00.cafe24.com/GetPrevRecords.php', {
				recordNo: recordNo,
			})
			.then(async response => {
	...
	_deleteDatas = async response => {
		for (const item of response.data) {
			await axios.post('http://dkstkdvkf00.cafe24.com/DeletePrevRecords.php', {
	...
	_input1 = async () => {
		...
		await Promise.all(
			Object.values(images).map(
				async image => await t._inputDatas(image.image, image.comment, image.createdAt, imageRoom)
			)
		);
	};
